...
- **Auto-created artifacts**: `pkg-cache/` and the `rootfs/` directory contents are created by `sbxctl`. They are root-managed (require `sudo` for modification/deletion).
...
- `pkg-cache/` may manually be pruned (e.g. `rm pkg-cache/*.deb`) or deleted to reduce usage of storage.
